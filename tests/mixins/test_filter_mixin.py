...
async def test_filters__params_empty__not_filtered(db_create_workers):
...
async def test_filters__params_filter__filter_with_params(model_filters, params):
...
@pytest.mark.usefixtures("db_fill")
async def test_filters__params_exclude__exclude_with_params(model_filters, params):
    workers_ids = await Worker.exclude(**model_filters).order_by("id").values_list("id", flat=True)
    assert workers_ids
...
@pytest.mark.usefixtures("db_fill")
async def test_filters__params_filter_and_exclude__filtered():
    worker = await Worker.first()

    response = await client.get("/", params={"name": worker.name, "id!": worker.id})
...
async def test_filters__param_not_in_model__ignored():
    workers_ids = await Worker.all().order_by("id").values_list("id", flat=True)
    assert workers_ids
...
async def test_filters__param_wrong_type__error(params):
    response = await client.get("/", params=params)
...
async def test_filters__param_list_element_wrong_type__error(params):
    response = await client.get("/", params=params)
