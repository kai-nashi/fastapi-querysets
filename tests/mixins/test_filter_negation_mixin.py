...
async def test_filter_negation_mixin__params_empty__not_filtered(db_create_workers):
...
async def test_filter_negation_mixin__params_exclude__exclude_with_params(model_filters, params):
...
async def test_filter_negation_mixin__param_not_in_model__ignored():
    workers_ids = await Worker.all().order_by("id").values_list("id", flat=True)
    assert workers_ids
...
async def test_filter_negation_mixin__param_wrong_type__error(params):
    response = await client.get("/", params=params)
...
async def test_filter_negation_mixin__param_list_element_wrong_type__error(params):
    response = await client.get("/", params=params)
